...
def adjust_to_ref_peak(df, ref_coords, tolerance_window=[0, 0], adjust_ref_peak_height_bool=False, ref_peak_target_height=1E6):
    ...
    adjust_ref_peak_height_bool: bool; whether or not to adjust ref peak ht. Set to False by default. 
...
def normxcorr2(template, image, mode="full"):
    ...
    Not interested in N > 2, though.
    This only works if nrows = 257 (i.e. J-res dimension form the rows)
...
def normxcorr_1d_fast_search(red_sum_loz, blue_sum_loz, stepsize=3, neighbourhood_size_padding=1):
    ...
    blue_sum_loz: array of float. len(blue_sum_loz) must be >= len(red_sum_loz), which should be ensured by having a nonzero search_region_padding_size upstream of this function. 
    ...
    RETURNS
    =======
    rho_ls: list of float; list of correlation values. 
    max_rho: max of rho_ls
    """
    rho_ls = [] # init
    max_rho = -1
    idx_of_max = -1
...
def do_2d_std_search(query_df, 
    ...
    """
    Wraps normxcorr2 for convenience. Written to mirror do_1d_std_search()
...
def get_starting_template(template_df, mcoords, ppm_shift, intensity_init=5E7):
    """
    Extracts a slice of `template_df`, where ppm lies within mcoords, with ppm shifted by `ppm_shift` and intensity scaled such that max intensity is set to `intensity_init`

    
    PARAMS
    ------
    template_df: spectra df, typically a full spectra df (of len 131072)
    ...
    intensity_init: float; 
    
    RETURNS
    -------
    dt: subset of template_df, with intensity scaled and ppm_shifted as desired
    """
    dt = template_df.copy()
...
def align_spectra_dfs(df1, df2):
    ...
    * both dfs may be trimmed until they have the same length, since they must be merged into a final df.
    ...
    PARAMS
    ------
    df1: df of spectra, with `intensity` and `ppm` columns.
    df2: df of spectra, with `intensity` and `ppm` columns.
...
def get_correlation_series(red_df, blue_df, 
    ...
    RETURNS
    -------
    d_corr: df with columns `ppm` and `corr_series`.
    """
    # align spectra
